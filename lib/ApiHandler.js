...
class ApiHandler {
    ...
    try {
      switch (method) {
        case 'account':
          ...
          break
        case 'freezes':
          ret = await this.api.query.balances.freezes(accountId)
          ret = { freezes: ret.toJSON() }
          break
        case 'holds':
          ret = await this.api.query.balances.holds(accountId)
          ret = { holds: ret.toJSON() }
          break
        case 'locks':
          ret = await this.api.query.balances.locks(accountId)
          ret = { locks: ret.toJSON() }
          break
        case 'reserves':
          ret = await this.api.query.balances.reserves(accountId)
          ret = { reserves: ret.toJSON() }
          break
        case 'totalIssuance':
          ret = await this.api.query.balances.totalIssuance()
  ...
  async nominationPools (method, params={}) {
    ...
    var ret
    try {
      var { id, pid=0, accountId } = params
      switch (method) {
        case 'lastPoolId':
          var ret = await this.api.query.nominationPools.lastPoolId()
          ret = { lastPoolId: ret.toNumber() }
          break
        case 'bondedPools':
          const bondedPools = await this.api.query.nominationPools.bondedPools(pid)
          ret = { bondedPools: bondedPools.toJSON() }
          break
        case 'metadata':
          // count(chain, '/query/nominationPools/metadata')
          const name = await this.api.query.nominationPools.metadata(pid)
          ret = { metadata: hexToString(name.toString()) }
          break
        case 'poolMembersForAccount':
          ret = await this.api.query.nominationPools?.poolMembers(accountId)
          ret = { poolMembers: ret ? ret.toJSON(): {} }
          break
        case 'poolMembers':
          // count(chain, '/query/nominationPools/poolMembers')
          var entries = await this.api.query.nominationPools.poolMembers.entries()
        ...
        case 'rewardPools':
          ...
          const rewardPools = await this.api.query.nominationPools.rewardPools(pid)
          ret = { rewardPools: rewardPools.toJSON() }
          break
        case 'subPoolsStorage':
          // count(chain, '/query/nominationPools/subPoolsStorage')
          const subPoolsStorage = await this.api.query.nominationPools.subPoolsStorage(pid)
